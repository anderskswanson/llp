...
/*-------------------------------------------------------------------------
 * Capability to the system ticks
 */
...
static inline struct TimerCap* isTimerCap(struct Cap* cap) {
  return (cap->type==TimerCap) ? (struct TimerCap*)cap : 0;
}

static inline void timerCap(struct Cap* cap, unsigned bits) {
  struct TimerCap* timecap = (struct TimerCap*)cap;
  printf("TimerCap at %x\n", timecap);  
  timecap->bits = bits;
  timecap->type = TimerCap;
}
