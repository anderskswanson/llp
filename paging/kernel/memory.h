...
#define pageNext(x)       ((alignTo(x,12))+0x1000)
//zero right bits and add one to leftmost bit
...
#define firstPageAfter(x) (maskTo(x, 12) | 0x000 ? pageNext(x) : x)
//pageNext iif not page start; else x
...
// = 0x1fff.
#define endPageBefore(x)  ((maskTo(x, 12) == 0xfff) ? x : pageEnd(x) - 0x1000) 
//pageEnd-1 iff not page end; else x
